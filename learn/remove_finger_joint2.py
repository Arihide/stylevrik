...
def nodes(fp, node, indent):
    ...
    if indent:
        if node.name in ['RightLeg', 'LeftLeg']:
            fp.write('%s\tCHANNELS 1 %s\n' % (indent_str, node.channels[1]))
        else:
...
if __name__ == "__main__":
    ...
    with open('%s_rmfinger2%s' % (root, ext), 'w') as ftarget:
